...
public class MoveFunctionPartidaFourierRodilla : MoveFunction {
	...
	public MoveFunctionPartidaFourierRodilla(float a1,float a2, float b1, float b2, float period, float fase, float centerAngle, float strength,
	...
	public override float evalAngle(float t){
		if(t<(2*Mathf.PI/B)){
			return  A*(float)Mathf.Sin(t*B+C) + D;
		}else{
