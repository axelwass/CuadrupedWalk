...
public class PushSimulationManager : SimulationManager {
		...
		void endActualTest(){
			float evaluation = tester.getAdvance()/*250f - tester.getCuadraticErrorPosition()*/ - tester.getCuadraticErrorRotation()/10;// / (1 + tester.getCuadraticError());
			evaluation = evaluation<0? 0: evaluation;
		//Debug.Log("test number: " + testNumber + "= speed: " + tester.getSpeed());
